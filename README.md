...
- **Model dropdown per row** — `(none)` / `default` / `opus` / `sonnet` / `haiku` / `fable`, matching Claude Code's `/model` picker. `(none)` leaves the session on whatever model it's already using; any other value prefixes the fire sequence with `/model <name>` + Enter so the session switches before continuing. Persisted per stable window title.
...
- **Fable refusal-recovery** (v1.0.16+, **opt-in, off by default**) — when a model's safeguards block a turn, Claude Code either offers a two-option picker (first option = switch to a fallback model) or switches by itself. Either way the session ends up off your chosen model. For each eligible window this accepts the picker if there is one, waits, then switches back with `/model`. Steps are editable, one per line: plain text is typed + Enter, `<confirm>` accepts whichever confirmable modal is showing (safeguard picker or the `/model` *Switch model?* dialog) and does nothing if there is none, `<wait>` / `<wait:N>` pauses — and it banks only time the session is actually **running**, so a network outage costs the recovery nothing. `<esc>` exists to surface a `/model` queued behind a busy turn but is **not** in the default script: it interrupts that turn, and in practice the turn is your real work. Every step is bounded, and completion reports which model the session actually ended on, warning loudly if it is not the one the script asked for. Status shows **⇄ Fable-recover**. It also **keeps the window on the target model**: if the session ends up on the fallback with no safeguard notice showing — a switch-back that never landed, or Claude Code switching by itself — the watchdog steers it back with `/model`, after a grace period, spaced out and capped so it never fights you. The target is simply the last `/model` step in your script; a script without one enforces nothing.
...
- **Fable-recovery is eligible on any watched terminal by default** ("Apply to all watched windows"). Only the window actually showing the safeguard notice is ever touched, but `find_terminal_windows()` filters by WT window class, not by "is this a Claude Code session" — so a plain shell that happens to *display* the notice wording (e.g. `cat`-ing a log or this project's own tests) would qualify. The detection patterns are deliberately de-fanged in this repo's own source, but untick "all windows" and pick your sessions if you keep unrelated terminals open.
