...
- **Fable refusal-recovery** (v1.0.16+, **opt-in, off by default**) — when a model's safeguards block a turn, Claude Code pauses the session and offers a two-option picker whose first entry (switch to a fallback model) is pre-selected. For each eligible window this accepts that option with a single Enter, waits, then switches back with `/model`. Steps are editable, one per line: plain text is typed + Enter, `<confirm>` waits for either confirmable modal (the safeguard picker or the `/model` *Switch model?* dialog) then accepts it, `<esc>` surfaces a dialog queued behind a busy turn (skipped when one is already showing, since ESC would dismiss it), `<wait>` / `<wait:N>` pauses. Every step is bounded — failed sends retry then abandon, `<confirm>` sends exactly one Enter per modal and has a hard deadline, waits are capped, and repeated refusals stop after two attempts — so a stuck recovery can never pin a window. Status shows **⇄ Fable-recover**.
