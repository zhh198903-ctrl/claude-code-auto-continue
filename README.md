...
- **Fable refusal-recovery** (v1.0.16+, **opt-in, off by default**) — when Fable's safeguards block a turn, the session stalls *on Fable*. For each eligible window this runs an editable step script (default: switch to Opus → confirm the *Switch model?* dialog → `continue` → wait → switch back to Fable → `continue`). Steps are one per line: plain text is typed + Enter, `<confirm>` waits for the switch dialog then accepts it, `<esc>` surfaces a dialog queued behind a busy turn (skipped if one is already showing, since ESC would cancel it), `<wait>` / `<wait:N>` pauses. Every step is bounded — failed sends retry then abandon, the confirm step has a hard deadline, waits are capped, and repeated refusals stop after two attempts — so a stuck recovery can never pin a window. Status shows **⇄ Fable-recover**. Requires Claude Code `/config` → *Switch models when a message is flagged* = **false**, or Claude Code switches models itself and there is no notice to detect.

All settings (poll interval, buffer, dry-run, keep-awake, start-on-launch, excluded windows, per-window model/effort, trigger-pattern overrides, Fable-recovery config) persist via `QSettings` (Windows registry under `HKCU\Software\auto_continue\gui`). Excluded windows, the model/effort overrides and the Fable per-window opt-in are keyed by the *stable* part of the WT title (leading spinner glyph stripped), so they survive title churn while a session is running.
